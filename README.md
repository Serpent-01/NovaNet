...
以下测试均在 Intel Core i5-9300H、本机 TCP loopback（回环网络）环境下完成。服务端和压测客户端运行于同一台机器，测试结果主要用于验证NovaNet在固定环境下的吞吐、延迟及长时间运行稳定性。

### Calculator.Add：Unary RPC

对`Calculator.Add`进行了64并发、约40分钟的稳定性测试。
...
测试期间未出现RPC失败、超时、进程崩溃或异常退出。

64个客户端连接建立后，服务端文件描述符由20增至84；测试结束并断开连接后恢复至20，未观察到文件描述符泄漏。服务端RSS在持续压测过程中保持稳定，未观察到持续增长。

#### perf分析

`perf`采样结果显示，主要CPU开销集中在：
...
在当前轻量级Unary RPC场景下，业务计算开销较低，整体性能主要受响应发送和本机TCP网络栈开销影响。

### Chat.Generate：Server Streaming RPC

Chat测试使用本地`FakeAiProvider`生成固定流式响应。每次`Chat.Generate`调用返回10个`GenerateChunk`，平均响应数据量约1079字节。

对`Chat.Generate`进行了64并发、约69分钟的稳定性测试。
...
| 测试时长 | 约69分钟 |
| 完成流数量 | 6400万个 |
| 平均响应分片 | 10 chunks/stream |
| 平均响应数据量 | 1079 bytes/stream |
| 成功率 | 100% |
| 持续吞吐 | 约15.46K streams/s |
| 平均延迟 | 约4.13 ms |
| P99延迟 | 9.74 ms |

测试期间未出现stream失败、超时、进程崩溃或异常退出，服务端线程数、RSS和文件描述符数量未观察到持续增长。

本次测试累计完成约6400万个Server Streaming调用，客户端共接收约6.4亿个`GenerateChunk`，用于验证以下流程：

- `STREAM_OPEN`、`STREAM_DATA`和`STREAM_END`处理；
- `stream_id`流式会话管理；
- 多个流之间的响应隔离；
- `ClientReader::Read()`和`Finish()`完整流程；
- 长时间运行下的连接与流资源回收。

### TCP_NODELAY优化

早期Chat压测中，服务端accepted socket未启用`TCP_NODELAY`。小型流式数据帧受到Nagle算法与delayed ACK（延迟确认）共同影响，导致CPU利用率较低且延迟明显升高。

服务端连接启用`TCP_NODELAY`后，在64并发环境下：

| 指标 | 优化前 | 优化后 |
|---|---:|---:|
| Chat吞吐 | 约1.44K streams/s | 约15.46K streams/s |
| 平均延迟 | 约44.38 ms | 约4.13 ms |

Chat吞吐提升约10.7倍，平均延迟降低约90.7%。

### 测试结论

- Unary RPC和Server Streaming在长时间测试中均保持100%成功率；
- 服务端RSS、线程数和文件描述符未出现持续增长；
- 连接关闭后文件描述符能够恢复至测试前水平；
- Chat能够持续完成流创建、分片传输、流结束及资源回收；
- 当前轻量级RPC场景的主要性能开销集中在响应发送和Linux TCP网络栈；
- 对小型RPC数据帧启用`TCP_NODELAY`能够显著降低延迟。

> 以上结果为Intel i5-9300H单机TCP loopback环境下的微基准测试，用于性能回归、稳定性验证及问题定位，不代表跨机器网络或真实生产业务环境的性能。