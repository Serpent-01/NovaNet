...
public:
    ...
    /*
     * 返回当前连接 outputBuffer_ 中尚未写入 socket 的字节数。
     *
     * 线程模型：
     * - 推荐只在连接所属 EventLoop 线程调用；
     * - RpcServerStreamResponder 会通过 queueInLoop 回到所属 loop 后调用；
     * - 不建议 worker 线程直接调用。
     */
    [[nodiscard]] std::size_t outputBufferSize() const;
